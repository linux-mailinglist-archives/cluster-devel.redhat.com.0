...
Received: from us-smtp-delivery-124.mimecast.com (us-smtp-delivery-124.mimecast.com [170.10.133.124])
	by mail.lfdr.de (Postfix) with ESMTP id DCDB43814AA
	for <lists+cluster-devel@lfdr.de>; Sat, 15 May 2021 02:36:21 +0200 (CEST)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=redhat.com;
	s=mimecast20190719; t=1621038981;
	...
	bh=xm1HqT7QxYGtgiqBXmqFanp09gtvXTk8r+8P7qZsNbk=;
	b=VfOfUx+d1tu+vR8+Rt8+MEijL0X8yalUY6Ojjc0hKL7mr9L7DYY/ymO0hDl3bABwd5c/8X
	11dvvqShG0/maNUfpoiKNSJbdxKWLYb+LToVv/Zd0plAksrW+QYtvso75lDwtdqniesxEe
	vzuf/sU3TZXC/bxCmWp3VGrP1C/JHHk=
Received: from mimecast-mx01.redhat.com (mimecast-mx01.redhat.com
 [209.132.183.4]) (Using TLS) by relay.mimecast.com with ESMTP id
 us-mta-412-eB_o5v6bPXm-7EUUxzgUSQ-1; Fri, 14 May 2021 20:36:18 -0400
X-MC-Unique: eB_o5v6bPXm-7EUUxzgUSQ-1
Received: from smtp.corp.redhat.com (int-mx02.intmail.prod.int.phx2.redhat.com [10.5.11.12])
	(using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
	(No client certificate requested)
	by mimecast-mx01.redhat.com (Postfix) with ESMTPS id 3F07D80DE13;
	Sat, 15 May 2021 00:36:17 +0000 (UTC)
Received: from colo-mx.corp.redhat.com (colo-mx01.intmail.prod.int.phx2.redhat.com [10.5.11.20])
	by smtp.corp.redhat.com (Postfix) with ESMTPS id 2FC19137D8;
	Sat, 15 May 2021 00:36:17 +0000 (UTC)
Received: from lists01.pubmisc.prod.ext.phx2.redhat.com (lists01.pubmisc.prod.ext.phx2.redhat.com [10.5.19.33])
	by colo-mx.corp.redhat.com (Postfix) with ESMTP id 20101180B463;
	Sat, 15 May 2021 00:36:17 +0000 (UTC)
Received: from smtp.corp.redhat.com (int-mx04.intmail.prod.int.phx2.redhat.com
	[10.5.11.14])
	by lists01.pubmisc.prod.ext.phx2.redhat.com (8.13.8/8.13.8) with ESMTP
	id 14F0aEsS030943 for <cluster-devel@listman.util.phx.redhat.com>;
	Fri, 14 May 2021 20:36:14 -0400
Received: by smtp.corp.redhat.com (Postfix)
	id 70B335DDAD; Sat, 15 May 2021 00:36:14 +0000 (UTC)
Delivered-To: cluster-devel@redhat.com
Received: from carbon.redhat.com (ovpn-115-135.rdu2.redhat.com [10.10.115.135])
	by smtp.corp.redhat.com (Postfix) with ESMTP id BFF055D9E3;
	Sat, 15 May 2021 00:36:13 +0000 (UTC)
From: Alexander Aring <aahringo@redhat.com>
To: teigland@redhat.com
Date: Fri, 14 May 2021 20:35:47 -0400
Message-Id: <20210515003549.1118171-14-aahringo@redhat.com>
...
Subject: [Cluster-devel] [PATCHv5 v5.13-rc1 dlm/next 13/15] fs: dlm: add
...
---
 ...
 fs/dlm/lowcomms.c     |    4 +-
 fs/dlm/lowcomms.h     |    7 +-
 fs/dlm/midcomms.c     | 1313 +++++++++++++++++++++++++++++++++++++++--
 fs/dlm/midcomms.h     |    1 +
 fs/dlm/rcom.c         |    4 +-
 7 files changed, 1315 insertions(+), 51 deletions(-)
...
index d9b7dafcf924..11d0f47a1ceb 100644
--- a/fs/dlm/lowcomms.c
+++ b/fs/dlm/lowcomms.c
@@ -1789,8 +1789,10 @@ static void process_send_sockets(struct work_struct *work)
...
index 8d74bb2e53e1..87fdbc599571 100644
--- a/fs/dlm/lowcomms.h
+++ b/fs/dlm/lowcomms.h
@@ -12,7 +12,12 @@
...
index aadb3781bebe..9600fc19f903 100644
--- a/fs/dlm/midcomms.c
+++ b/fs/dlm/midcomms.c
@@ -3,7 +3,7 @@
 *******************************************************************************
...
+#define DLM_NODE_FLAG_REMOVED	4
+	unsigned long flags;
...
+				 */
+				if (!node->users || test_bit(DLM_NODE_FLAG_REMOVED,
+							     &node->flags)) {
...
+				switch (node->state) {
+				case DLM_CLOSE_WAIT:
+					/* saw that case once, hope timeout
+					 * handling deals with it.
+					 */
+					fallthrough;
+				case DLM_LAST_ACK:
+					/* we probably didn't got a last ack back from last fin,
+					 * so we reset the node.
+					 *
+					 * This the timeout handling for passive shutdown EOF.
+					 */
+					log_print("reset node %d because timeout handling",
+						  node->nodeid);
+
+					/* don't flush lowcomms here */
...
+	dlm_receive_buffer(p, nodeid);
 ...
 /*
@@ -101,32 +878,19 @@ int dlm_process_incoming_buffer(int nodeid, unsigned char *buf, int len)
 		if (msglen > len)
 			break;
...
-skip:
 		...
 		len -= msglen;
 		ptr += msglen;
@@ -134,3 +898,464 @@ int dlm_process_incoming_buffer(int nodeid, unsigned char *buf, int len)
...
+	 */
+	if (!node->users) {
+		set_bit(DLM_NODE_FLAG_REMOVED, &node->flags);
+
...
+		 node->nodeid, dlm_state_str(node->state));
+	dlm_send_queue_flush(node);
+	dlm_lowcomms_flush(node->nodeid, true);
+}
+
+void dlm_midcomms_shutdown(void)
+{
