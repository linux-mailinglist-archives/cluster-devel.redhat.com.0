...
Received: from us-smtp-delivery-1.mimecast.com (us-smtp-2.mimecast.com [207.211.31.81])
	by mail.lfdr.de (Postfix) with ESMTP id C424F180F2F
	for <lists+cluster-devel@lfdr.de>; Wed, 11 Mar 2020 06:07:08 +0100 (CET)
DKIM-Signature: v=1; a=rsa-sha256; c=relaxed/relaxed; d=redhat.com;
	s=mimecast20190719; t=1583903227;
	h=from:from:sender:sender:reply-to:subject:subject:date:date:
	 message-id:message-id:to:to:cc:cc:mime-version:mime-version:
	 content-type:content-type:
	 content-transfer-encoding:content-transfer-encoding:list-id:list-help:
	 list-unsubscribe:list-subscribe:list-post;
	bh=fYcVlBUWvbyor4ui+M2DUGTLgZCbM4lWXfNRJoOvko4=;
	b=Y+Q5bsXsNqu9aU7wjp6ahDkBa7mX7Lh3j1yPuxqRFRQyWyUyMqg8f0/RUweeRHhTgVtees
	RnJsbTNEy4N3ZCo9mTlDYH6jGZImw4lRDuC5CSuU8Zwul8gyTXzuIZrfEcMT4U6FBt6QB6
	5sDn2Qikldwkr7e2KcleSJ1jt6QFbds=
Received: from mimecast-mx01.redhat.com (mimecast-mx01.redhat.com
 [209.132.183.4]) (Using TLS) by relay.mimecast.com with ESMTP id
 us-mta-203-RcF-wgxSNRu4q5hs47xCDw-1; Wed, 11 Mar 2020 01:07:04 -0400
X-MC-Unique: RcF-wgxSNRu4q5hs47xCDw-1
Received: from smtp.corp.redhat.com (int-mx07.intmail.prod.int.phx2.redhat.com [10.5.11.22])
	(using TLSv1.2 with cipher AECDH-AES256-SHA (256/256 bits))
	(No client certificate requested)
	by mimecast-mx01.redhat.com (Postfix) with ESMTPS id 957B2477;
	Wed, 11 Mar 2020 05:07:01 +0000 (UTC)
Received: from colo-mx.corp.redhat.com (colo-mx01.intmail.prod.int.phx2.redhat.com [10.5.11.20])
	by smtp.corp.redhat.com (Postfix) with ESMTPS id 515271001920;
	Wed, 11 Mar 2020 05:06:59 +0000 (UTC)
Received: from lists01.pubmisc.prod.ext.phx2.redhat.com (lists01.pubmisc.prod.ext.phx2.redhat.com [10.5.19.33])
	by colo-mx.corp.redhat.com (Postfix) with ESMTP id A3D9018089CE;
	Wed, 11 Mar 2020 05:06:56 +0000 (UTC)
Received: from smtp.corp.redhat.com (int-mx04.intmail.prod.int.rdu2.redhat.com
	[10.11.54.4])
	by lists01.pubmisc.prod.ext.phx2.redhat.com (8.13.8/8.13.8) with ESMTP
	id 02B56lUk027089 for <cluster-devel@listman.util.phx.redhat.com>;
	Wed, 11 Mar 2020 01:06:47 -0400
Received: by smtp.corp.redhat.com (Postfix)
	id E509F2028CD2; Wed, 11 Mar 2020 05:06:46 +0000 (UTC)
Delivered-To: cluster-devel@redhat.com
Received: from mimecast-mx02.redhat.com
	(mimecast05.extmail.prod.ext.rdu2.redhat.com [10.11.55.21])
	by smtp.corp.redhat.com (Postfix) with ESMTPS id DD3CC2026D67
	for <cluster-devel@redhat.com>; Wed, 11 Mar 2020 05:06:44 +0000 (UTC)
Received: from us-smtp-1.mimecast.com (us-smtp-2.mimecast.com [205.139.110.61])
	(using TLSv1.2 with cipher ECDHE-RSA-AES256-SHA384 (256/256 bits))
	(No client certificate requested)
	by mimecast-mx02.redhat.com (Postfix) with ESMTPS id E461780029A
	for <cluster-devel@redhat.com>; Wed, 11 Mar 2020 05:06:43 +0000 (UTC)
...
X-MIME-Autoconverted: from quoted-printable to 8bit by
	lists01.pubmisc.prod.ext.phx2.redhat.com id 02B56lUk027089
...
Subject: [Cluster-devel] [PATCH -next 000/491] treewide: use fallthrough;
...
Link: https://lore.kernel.org/lkml/b56602fcf79f849e733e7b521bb0e17895d390fa=
.1582230379.git.joe.com/

These patches are based on next-20200310 and are available in

git://repo.or.cz/linux-2.6/trivial-mods.git in branch 20200310_fallthrough_=
2
...
sub file_before_directory {
    ...
    my $count =3D scalar(@$array_ref);

    for (my $i =3D 1; $i < $count; $i++) {
=09if (substr(@$array_ref[$i - 1], -1) eq '/' &&
=09    substr(@$array_ref[$i], 0, length(@$array_ref[$i - 1])) eq @$array_r=
ef[$i - 1]) {
=09=09my $string =3D @$array_ref[$i - 1];
=09=09@$array_ref[$i - 1] =3D @$array_ref[$i];
=09=09@$array_ref[$i] =3D $string;
=09    }
=09}
}
...
my $maintainer_patterns =3D `grep -P '^F:\\s+' MAINTAINERS`;
my @patterns =3D split('\n', $maintainer_patterns);
s/^F:\s*// for @patterns;
@patterns =3D grep(!/^(?:Documentation|tools|scripts)\//, @patterns);
@patterns =3D grep(!/\.(?:dtsi?|rst|config)$/, @patterns);
@patterns =3D sort @patterns;
@patterns =3D sort { $b =3D~ tr/\//\// cmp $a =3D~ tr/\//\// } @patterns;
...
# Find the files the pattern matches
    my $pattern_files =3D `git ls-files -- $pattern`;
    my @new_patterns =3D split('\n', $pattern_files);
    $pattern_files =3D join(' ', @new_patterns);
    next if ($pattern_files =3D~ /^\s*$/);

# Find the section the first file matches
    my $pattern_file =3D @new_patterns[0];
    my $section_output =3D `./scripts/get_maintainer.pl --nogit --nogit-fal=
lback --sections --pattern-depth=3D1 $pattern_file`;
    my @section =3D split('\n', $section_output);
    my $section_header =3D @section[0];

    print("Section: <$section_header>\n");

# Skip the section if it's already done
    print("Already done '$section_header'\n") if ($sections_done{$section_h=
eader});
...
=09last if ($line =3D~ /^\s*$/);
=09push(@new_section, $line);
    }
    @section =3D grep(/^F:/, @new_section);
    s/^F:\s*// for @section;

    @section =3D grep(!/^(?:Documentation|tools|scripts)\//, @section);
    @section =3D grep(!/\.(?:dtsi?|rst|config)$/, @section);
    @section =3D sort @section;
    @section =3D uniq(@section);

    my $section_files =3D join(' ', @section);

    print("section_files: <$section_files>\n");

    next if ($section_files =3D~ /^\s*$/);

    my $cvt_files =3D `git ls-files -- $section_files`;
    my @files =3D split('\n', $cvt_files);

    @files =3D grep(!/^(?:Documentation|tools|scripts)\//, @files);
    @files =3D grep(!/\.(?:dtsi?|rst|config)$/, @files);
    @files =3D grep(/\.[ch]$/, @files);
    @files =3D sort @files;
    @files =3D uniq(@files);

    $cvt_files =3D join(' ', @files);
...
Done via script
Link: https://lore.kernel.org/lkml/b56602fcf79f849e733e7b521bb0e17895d390fa=
.1582230379.git.joe@perches.com/
EOF
;
    close $fh;
